...
import fr.free.nrw.commons.jpegtran.AJpegTran;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button_grayscale = findViewById(R.id.button_grayscale);
        button_grayscale.setOnClickListener(this);
        Button button_flip = findViewById(R.id.button_flip);
        button_flip.setOnClickListener(this);
        ...
        if( loadUri == null ) {
            ...
            button_grayscale.setVisibility(View.INVISIBLE);
            button_flip.setVisibility(View.INVISIBLE);
            button_rotate.setVisibility(View.INVISIBLE);
        }
                        ...
                        if(result.getResultCode() == RESULT_OK ){
                            ...
                            if(data != null) {
                                ...
                                if (jniresult.startsWith("OK")) {
                                    ...
                                    button_grayscale.setVisibility(View.VISIBLE);
                        ...
                        if(result.getResultCode() == RESULT_OK ){
                            ...
                            if(data != null) {
                                ...
                                try {
                                    ...
                                    // When you implement your app, you should make thread and call ajpegtran() from the thread.
                                    //
                                    // Use the options selected by the user
                                    String options = getTransformOptions();
                                    jniresult = AJpegTran.ajpegtran(rparcelFd.detachFd(), wparcelFd.detachFd(), options);
                                } catch (IOException e) {
    ...
    private void applyTransformations(){
        if (loadUri == null) return;
        executorService.execute(()->{
            try{
                File tempFile = new File(getCacheDir(),"preview.img");
                ParcelFileDescriptor rpf = getContentResolver().openFileDescriptor(loadUri,"r");
                ParcelFileDescriptor wpf = ParcelFileDescriptor.open(tempFile,ParcelFileDescriptor.MODE_WRITE_ONLY|ParcelFileDescriptor.MODE_CREATE|ParcelFileDescriptor.MODE_TRUNCATE);

                String options = getTransformOptions();
                String result  = AJpegTran.ajpegtran(rpf.detachFd(), wpf.detachFd(), options);

                if (result.startsWith("OK")) {
                      // Decode the temporary file into a Bitmap
                    ...
                    runOnUiThread(() -> Toast.makeText(getApplicationContext(), "Error: " + errorMsg, Toast.LENGTH_LONG).show());
                }
            }catch (IOException e){
                runOnUiThread(() -> Toast.makeText(getApplicationContext(),"Error while applying transformations",Toast.LENGTH_LONG).show());
            }
                }
        );
    ...
    private String getTransformOptions() {
        StringBuilder options = new StringBuilder();
        if (isGrayscale) {
            options.append("-grayscale ");
        }
        if (isFlipHorizontal) {
            options.append("-flip horizontal ");
        }
        if (rotationAngle != 0) {
            options.append("-rotate ").append(rotationAngle).append(" ");
        }
        options.append("-optimize -copy all");
