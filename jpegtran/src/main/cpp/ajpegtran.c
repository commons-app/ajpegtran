    ...
    if (setjmp(jbuf) == 0) {
        ...
        /* Set return array */
        {
            ...
            for (cnt = 1; cnt < srcinfo.num_components; cnt++) {
                if (hmax < srcinfo.comp_info[cnt].h_samp_factor)
                    hmax = srcinfo.comp_info[cnt].h_samp_factor;
                if (vmax < srcinfo.comp_info[cnt].v_samp_factor)
                    vmax = srcinfo.comp_info[cnt].v_samp_factor;
            }
            /* Check array size */
